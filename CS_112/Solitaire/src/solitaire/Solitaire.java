...
public class Solitaire {
	...
	void jokerA() {
		...
		CardNode after = tmp.next;
		if(tmp.cardValue == 27){
			while(tmp.next!=deckRear){
				tmp = tmp.next;
			}
			tmp.next = deckRear.next;
			deckRear.next = tmp.next.next;
			tmp.next.next = deckRear;
			deckRear = tmp.next;
		}
		else{
			while(tmp.next.cardValue != 27){
				tmp = tmp.next;
				after = tmp.next;				
			}
			tmp.next = after.next;
			after.next = tmp.next.next;
			tmp.next.next = after;
