...
public class Radixsort {
	...
	throws IOException {
		...
		while(sc.hasNextLine()){
			strtmp = sc.nextLine();
			Node<String> tmp = new Node<String>(strtmp,masterListRear.next);
	...
	public void scatter(int pass) {
		...
		do{
			...
			if ((ptr.data.length()-pass-1)<0){
				...
				radixvalue = Character.digit(ptr.data.charAt(ptr.data.length()-pass-1), radix);
				if (buckets[radixvalue] == null){
					buckets[radixvalue] = ptr;
					ptr.next = ptr;
				}
				else{
					ptr.next = buckets[radixvalue].next;
					buckets[radixvalue].next = ptr;
					buckets[radixvalue] = ptr;
				}
